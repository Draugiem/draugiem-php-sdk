<?php
/*
	This is a simple example script that shows how easy is to integrate your page with Draugiem ID.
...
if($session){ // Authentication successful
	...
	if($users = $draugiem->getUserFriends(1, 10)){
		...
		foreach($users as $friend){
			...
			echo '<a target="_top" href="https://www.draugiem.lv/friend/?'.$friend['uid'].'">';
	...
	echo '<h2>Welcome to our site. Please login with Draugiem ID</h2>';

	$redirect = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; // Where to redirect after authorization

	echo $draugiem->getDraugiemIDButton($redirect, false); // Show the button
